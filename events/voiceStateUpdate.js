...
module.exports = async (client, oldState, newState) => {
  try {
    // Validate states
    if (!oldState || !newState || !newState.guild) {
      return;
    }
    ...
    switch (stateChange.type) {
      ...
      case "LEAVE":
        ...
        if (player.get("autoPause") === true) {
          if (members === 0 && !player.paused && player.playing) {
            ...
            let playerPaused = new MessageEmbed()
              ...
              .setFooter({
                text: `The current song has been paused because there's no one in the voice channel. I'll stay here and wait!`,
              });
        ...
        // autoLeave is disabled in config, so this ensures bot stays forever
        client.warn(
          `Player: ${player.options.guild} | Everyone left, but staying in channel (24/7 mode)`
        );
        break;
    }
  } catch (error) {
    client.error("Error in voiceStateUpdate:", error);
  }
};
