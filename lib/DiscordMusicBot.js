...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    console.log("Spotify Client ID:", client.config.spotify?.clientId ? `✓ Set (${client.config.spotify.clientId.substring(0, 8)}...)` : "✗ Missing");
    console.log("Spotify Client Secret:", client.config.spotify?.clientSecret ? "✓ Set (hidden)" : "✗ Missing");
    ...
    if (client.config.spotify?.clientId && client.config.spotify?.clientSecret) {
      ...
      const clientSecretValid = client.config.spotify.clientSecret.length === 32;
      
      if (!clientIdValid) {
        console.warn("⚠️  Spotify Client ID appears invalid (should be 32 characters)");
      }
      if (!clientSecretValid) {
        console.warn("⚠️  Spotify Client Secret appears invalid (should be 32 characters)");
      }
      
      if (clientIdValid && clientSecretValid) {
        console.log("✓ Spotify plugin will initialize - token renewal messages are normal!");
    ...
    try {
      ...
      console.log("  convertUnresolved: true - Spotify tracks will auto-resolve");
    } catch (err) {
    ...
    this.manager = new Manager({
      ...
      .on("trackError", (player, track, payload) => {
        this.error(
          `Player: ${player.options.guild} | Track error: ${track?.title || 'Unknown'}`
        );
      ...
      .on("trackStuck", (player, track, threshold) => {
        this.error(
          `Player: ${player.options.guild} | Track stuck: ${track?.title || 'Unknown'} (${threshold}ms)`
        );
        console.error("Stuck track data:", track);
      })
      .on("searchFailed", (player, query, error) => {
        this.error(
          `Player: ${player?.options?.guild || "Unknown"} | Search failed for: ${query}`
        );
      ...
      .on(
        ...
        /** @param {EpicPlayer} player */ async (player, track) => {
          ...
          console.log(`[TRACK_START] Identifier: ${track.identifier || 'undefined'}`);

          this.warn(
            `Player: ${
              player.options.guild
            } | Track has been started playing [${colors.blue(track.title || 'Unknown')}]`
          );
          ...
          var trackUri = track.uri || track.url || `https://youtube.com/watch?v=${track.identifier || ''}`;
