  ...
  build() {
    ...
    this.manager = new Manager({
      ...
      .on("nodeConnect", (node) => {
        ...
        // Restore all players when node reconnects
        const players = [...this.manager.players.values()];
        if (players.length > 0) {
          this.log(
            `Node: ${node.options.identifier} | Restoring ${players.length} player(s) after reconnection...`
          );
          ...
          players.forEach((player) => {
            try {
              ...
              if (player.voiceChannel && !player.connected) {
                player.connect();
                this.log(
                  `Player: ${player.guild} | Reconnected to voice channel after node reconnection`
                );
              }
              ...
              if (player.queue.current) {
                ...
                this.log(
                  `Player: ${player.guild} | Resuming track "${player.queue.current.title}" at position ${position}ms`
                );
              ...
              this.warn(
                `Player: ${player.guild} | Failed to restore after reconnection: ${error.message}`
              );
            }
          });
        }
      ...
      .on("nodeDisconnect", (node) => {
        ...
        // Save player states before disconnect
        const players = [...this.manager.players.values()];
        players.forEach((player) => {
          if (player.playing && player.queue.current) {
            player.set("savedPosition", player.position);
            player.set("savedTrack", player.queue.current);
            this.log(
              `Player: ${player.guild} | Saved state for reconnection (position: ${player.position}ms)`
            );
          }
        });
      })
      .on("nodeError", (node, err) => {
        ...
        // Suppress "Unexpected op ready" errors as they're harmless during reconnection
        if (err.message && err.message.includes('Unexpected op "ready"')) {
          this.log(
            `Node: ${node.options.identifier} | Ignoring harmless reconnection message`
          );
          return;
        }
      })
      ...
      .on(
        ...
        /** @param {EpicPlayer} player */ async (player, track) => {
          ...
          // Update bot status with current track
          try {
            let statusText = "the echo";

            // Try to extract artist from title (format: "Artist - Song" or "Artist - Song Name")
            if (track.title) {
              const title = track.title;

              // Check if title contains " - " which usually separates artist from song
              if (title.includes(" - ")) {
                const parts = title.split(" - ");
                // Take the first part as artist name (before the dash)
                let artist = parts[0].trim();

                // Remove common YouTube suffixes like (Official Video), (Audio), etc.
              ...
              // If no dash, just use the full title
              else {
                statusText = title;
            ...
            // Update presence with new status text (add 🎶 emoji)
            ...
            console.error("[STATUS] Failed to update status:", err.message);
          }
          ...
          try {
            ...
            client.error("Failed to send now playing message:", err);
            console.error("[TRACK_START] Full error:", err);
            console.error("[TRACK_START] Error message:", err.message);
            console.error("[TRACK_START] Error stack:", err.stack);
          }
        }
      )
        ...
        /** @param {EpicPlayer} */ async (player, track) => {
          ...
          if (autoQueue) {
            ...
            try {
              ...
              if (res.exception) {
                throw new Error(res.exception.message || "Search failed");
              }
              ...
              // If all tracks were played, just use the first one
              if (!nextTrack && res.tracks.length > 0) {
              ...
              if (nextTrack) {
                ...
                client.log(
                  `AutoQueue: Added and playing "${nextTrack.title}" for player ${player.options.guild}`
                );
              } else {
              ...
              client.warn(
                `AutoQueue failed for player ${player.options.guild}: ${error.message}. Staying in channel (24/7 mode).`
              );

              // Reset status to default when autoQueue fails
              try {
                const currentPresence = client.config.presence || {};
                const currentActivity = currentPresence.activities?.[0] || {};
                ...
                console.log("[STATUS] Reset to: the echo (autoQueue failed)");
              } catch (err) {
            ...
            // Reset status to default "the echo" when queue ends
            try {
              const currentPresence = client.config.presence || {};
              const currentActivity = currentPresence.activities?.[0] || {};
              ...
              console.log("[STATUS] Reset to: the echo (queue ended)");
            } catch (err) {
              console.error("[STATUS] Failed to reset status:", err.message);
            }
